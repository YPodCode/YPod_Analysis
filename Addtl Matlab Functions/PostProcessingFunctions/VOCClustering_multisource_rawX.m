function [classObj, C_hat, X_hat, X_all] = VOCClustering_multisource_rawX(settingsSet)
        ...
        for m=1:1%nRegs
            ...
            for k=1:nVal
                ...
                for kk=1:nReps
                    ...
                    [X, C, t] = alignRefandPod(X_OG,xt,C_ref,ct,settingsSet);
                    
                    fprintf('j=%u, m=%u, k=%u, kk=%u, Y: [%u %u] C [%u %u]\n',[j m k kk size(Y) size(C)])
                    
                    valList{j,m,k,kk} = timeFold(C, X, t, settingsSet.nFolds);
                    ...
                    for zz = 1:nClass
                        ...
                        try
                            ...
                            disp('-----Fitting...');
                            cmodels{j,m,k,kk,zz,1} = fitFunc(C(tempvallist~=kk,:), X(tempvallist~=kk,:), settingsSet);
                            disp('-----Apply to training data...');
                            %Apply the fitted regression to the calibration data
                            C_hat.Y.cal{j,m,k,kk,zz} = predictFunc(X(tempvallist~=kk,:),cmodels{j,m,k,kk,zz,1},settingsSet);
                            disp('-----Apply to test data...');
                            %Apply the fitted regression to the validation data
                            C_hat.Y.val{j,m,k,kk,zz} = predictFunc(X(tempvallist==kk,:),cmodels{j,m,k,kk,zz,1},settingsSet);
                            close all
                        catch err
                            warning(['Problem with classification' newline getReport(err)])
                        end
...
temppath = fullfile(originalPath,'classResults_Raw');
save(char(temppath),'classObj','-v7.3');
temppath = fullfile(originalPath,'classModels_Raw');
